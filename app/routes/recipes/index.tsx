...
const RecipesPage = ({ loaderData}:Route.ComponentProps) => { //the data returned from the function above is passed into the arguments here.
    ...
    const totalPages = Math.ceil(searchResult.length / recipesPerPage);
    const indexOfLast = currentPage * recipesPerPage;
    const indexOfFirst = indexOfLast - recipesPerPage;
    const currentRecipes = searchResult.slice(indexOfFirst, indexOfLast);
    ...
    <>
        ...
        <Searchbar searchQuery={searchQuery} onSearchChange={(q) => {setSearchQuery(q); setCurrentPage(1)}}/>
